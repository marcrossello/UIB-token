...
MAX_RETRY="$3"
VERBOSE="$4"
: ${CHANNEL_NAME:="collaborationchannel"}
: ${DELAY:="3"}
: ${MAX_RETRY:="5"}
: ${VERBOSE:="false"}
...
createChannelTx() {
	set -x
	configtxgen -profile PrivateOrgsChannel -outputCreateChannelTx ./channel-artifacts/${CHANNEL_NAME}.tx -channelID $CHANNEL_NAME
	res=$?
	{ set +x; } 2>/dev/null
